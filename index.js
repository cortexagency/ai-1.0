...
const PORT_QR = process.env.PORT || 3000;
let lastQr = null;

// reuse existing app if exists
let appInstance = (typeof app !== 'undefined') ? app : express();

// add QR handlers without breaking existing ones
if (typeof client !== 'undefined') {
  client.on('qr', async (qr) => {
  ...
  if (lastQr) {
    res.send(`
      <html>
        ...
        <body style="font-family:sans-serif;text-align:center;margin-top:40px;">
          <h2>📱 Escanea este QR para vincular tu WhatsApp</h2>
          <img src="${lastQr}" style="width:300px;margin-top:20px;border:1px solid #ccc;border-radius:12px;" />
          <p style="margin-top:30px;color:gray;">Si ya lo escaneaste, espera a que se conecte…</p>
        </body>
      </html>
    `);
...
appInstance.get('/qr', (req, res) => {
  if (lastQr) res.redirect(lastQr);
  else res.send('No hay QR disponible todavía.');
});

// Start server safely if not started
if (!appInstance.listening) {
  appInstance.listen(PORT_QR, () => {
    console.log(`🌐 QR disponible en http://localhost:${PORT_QR} o tu Railway URL`);
    appInstance.listening = true;
  });
}
